...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for entry in orm.Entry.objects.all():
            orthvars = entry.orthographic_variants.all()
            reconstructed = all([o.is_reconstructed for o in orthvars])
            questionable = any([o.is_approved for o in orthvars])
            entry.reconstructed_headword = reconstructed
            entry.questionable_headword = questionable
            entry.save()
