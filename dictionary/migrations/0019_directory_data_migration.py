...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        from slavdict.dictionary.models import TANTUM_MAP
        from slavdict.dictionary.models import GENDER_MAP
        from slavdict.dictionary.models import ONYM_MAP
        from slavdict.dictionary.models import TRANSITIVITY_MAP
        from slavdict.dictionary.models import PARTICIPLE_TYPE_MAP
        from slavdict.dictionary.models import STATUS_MAP
        from slavdict.dictionary.models import LANGUAGE_MAP
        from slavdict.dictionary.models import SUBSTANTIVUS_TYPE_MAP
        # Note: Remember to use orm['appname.ModelName'] rather than "from appname.models..."

        for entry in orm['dictionary.Entry'].objects.all():

            slug = entry.part_of_speech and entry.part_of_speech.slug
            if slug:
                entry.DMG_part_of_speech = PART_OF_SPEECH_MAP[slug]

            slug = entry.tantum and entry.tantum.slug
            if slug:
                entry.DMG_tantum = TANTUM_MAP[slug]

            slug = entry.gender and entry.gender.slug
            if slug:
                entry.DMG_gender = GENDER_MAP[slug]

            slug = entry.onym and entry.onym.slug
            if slug:
                entry.DMG_onym = ONYM_MAP[slug]

            slug = entry.transitivity and entry.transitivity.slug
            if slug:
                entry.DMG_transitivity = TRANSITIVITY_MAP[slug]

            slug = entry.participle_type and entry.participle_type.slug
            if slug:
                entry.DMG_participle_type = PARTICIPLE_TYPE_MAP[slug]

            slug = entry.status and entry.status.slug
            if slug:
                entry.DMG_status = STATUS_MAP[slug]

            entry.save()

        for etymology in orm['dictionary.Etymology'].objects.all():
            etymology.DMG_language = LANGUAGE_MAP[etymology.language.slug]
            etymology.save()

        for meaning in orm['dictionary.Meaning'].objects.all():
            slug = meaning.substantivus_type and meaning.substantivus_type.slug
            if slug:
                meaning.DMG_substantivus_type = SUBSTANTIVUS_TYPE_MAP[slug]
                meaning.save()
