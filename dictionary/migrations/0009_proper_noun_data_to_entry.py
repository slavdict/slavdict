...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for proper_noun in orm.ProperNoun.objects.all():
            entry = proper_noun.entry
            entry.onym = proper_noun.onym
            entry.canonical_name = proper_noun.canonical_name
            entry.nom_sg = proper_noun.nom_sg
            entry.nom_pl = proper_noun.nom_pl
            entry.save()


    def backwards(self, orm):
        for entry in orm.Entry.objects.filter(onym__isnull=False):
            proper_noun = orm.ProperNoun()
            proper_noun.entry = entry
            proper_noun.onym = entry.onym
            proper_noun.canonical_name = entry.canonical_name
            proper_noun.nom_sg = entry.nom_sg
            proper_noun.nom_pl = entry.nom_pl
            proper_noun.save()
    ...
    complete_apps = ['dictionary', 'dictionary']
