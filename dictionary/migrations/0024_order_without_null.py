...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for x in ('Etymology', 'MeaningContext', 'Meaning', 'Example',
                  'Collocation', 'OrthographicVariant', 'Participle'):
            for i in orm['dictionary.{0}'.format(x)].objects.all():
                if i.order is None:
                    i.order = 0
                    i.save()

    def backwards(self, orm):
        raise RuntimeError('Backward migration is not possible')
