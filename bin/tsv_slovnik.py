...
LETTER = u'в'
FILENAME = '/root/slovnik_antconc_vedi.txt'
OUTPUT_FILENAME = '/root/vedi.tsv'
VS_FILENAME = '/root/vedi-versus.tsv'

glossary_freq = open(FILENAME).read().decode('utf-8').strip().split('\n')
glossary_freq = [l.split('\t') for l in glossary_freq]
glossary_freq = [(s, int(n)) for (s, n) in glossary_freq]
