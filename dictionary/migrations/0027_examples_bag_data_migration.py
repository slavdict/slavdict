...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for example in Example.objects.all():
            host_entry = example.host_entry
            example.entry = host_entry
            host = example.host
            if host and 'base_meaning_id' in host.__dict__:
                example.collogroup = host
            example.save(without_mtime=True)


    def backwards(self, orm):
        for example in orm.Example.objects.all():
            example.entry_set.add(example.entry)
            example.entry = None
            example.save()
